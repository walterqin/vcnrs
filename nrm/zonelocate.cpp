...
ZoneLocate::ZoneLocate(QObject *parent)
    : QObject(parent)
{
...
void ZoneLocate::deleteNotArea(Mat &inmat, Color color)
{
    Mat input_grey;
    ...
    if (findLeftRightBound1(img_threshold, posLeft, posRight))
    {
...
void ZoneLocate::setLifemode(bool param)
{
    if (param)
    {
...
bool ZoneLocate::verifySizes(RotatedRect mr)
{
    float error = m_error;
...
int ZoneLocate::mserSearch(const Mat &src, vector<Mat>& out,
               vector<vector<NumberZone>>& out_plateVec,
               bool usePlateMser,
...
int ZoneLocate::colorSearch(const Mat& src, const Color r, Mat& out, std::vector<RotatedRect>& outRects)
{
    Mat match_grey;
...
int ZoneLocate::sobelFrtSearch(const Mat &src, std::vector<Rect_<float>>& outRects)
{
    Mat src_threshold;
...
int ZoneLocate::sobelSecSearch(Mat& bound, Point2f refpoint, std::vector<RotatedRect>& outRects)
{
    Mat bound_threshold;
...
int ZoneLocate::sobelSecSearchPart(Mat& bound, Point2f refpoint, std::vector<RotatedRect>& outRects)
{
    Mat bound_threshold;
    ...
    if (findLeftRightBound(tempBoundThread, posLeft, posRight)) {
...
int ZoneLocate::sobelOper(const Mat& in, Mat& out, int blurSize, int morphW, int morphH)
{
    Mat mat_blur;
    mat_blur = in.clone();
...
int ZoneLocate::sobelOperT(const Mat& in, Mat& out, int blurSize, int morphW, int morphH)
{
    Mat mat_blur;
    mat_blur = in.clone();
    GaussianBlur(in, mat_blur, Size(blurSize, blurSize), 0, 0, BORDER_DEFAULT);
...
bool ZoneLocate::rotation(Mat& in, Mat& out, const Size rect_size, const Point2f center, const double angle)
{
    Mat in_large;
...
int ZoneLocate::deskew(const Mat& src, const Mat& src_b,
           std::vector<RotatedRect>& inRects, std::vector<NumberZone>& outPlates,
           bool useDeteleArea, Color color)
{
    Mat mat_debug;
    ...
    for (size_t i = 0; i < inRects.size(); i++) {
        ...
        if (roi_angle - m_angle < 0 && roi_angle + m_angle > 0) {
            ...
            if (deskew_mat.cols * 1.0 / deskew_mat.rows > 2.3 && deskew_mat.cols * 1.0 / deskew_mat.rows < 6) 
			{
                ...
                NumberZone plate;
                plate.setNumberZonePos(roi_rect);
                plate.setNumberZoneMat(plate_mat);
                if (color != UNKNOWN)
					plate.setNumberZoneColor(color);
...
bool ZoneLocate::isDeflection(const Mat& in, const double angle, double& slope)
{
    int nRows = in.rows;
    int nCols = in.cols;
...
void ZoneLocate::affine(const Mat& in, Mat& out, const double slope)
{
    Point2f dstTri[3];
    Point2f plTri[3];
...
int ZoneLocate::zoneColorLocate(Mat src, std::vector<NumberZone>& candPlates, int index)
{
    ...
    vector<NumberZone> plates_blue;
    plates_blue.reserve(64);
    vector<NumberZone> plates_yellow;
...
int ZoneLocate::zoneSobelLocate(Mat src, std::vector<NumberZone>& candPlates, int index)
{
    vector<RotatedRect> rects_sobel_all;
    rects_sobel_all.reserve(256);

    vector<NumberZone> plates;
    plates.reserve(32);
...
int ZoneLocate::zoneMserLocate(Mat src, std::vector<NumberZone>& candPlates, int index)
{
    ...
    for (size_t i = 0; i < channelImages.size(); ++i)
    {
        ...
        vector<vector<NumberZone>> platesVec;
        ...
        mserSearch(image, src_b_vec, platesVec, usePlateMser, plateRRectsVec, index, false);

        for (size_t j = 0; j < flags.size(); j++)
        {
            vector<NumberZone>& plates = platesVec.at(j);
            Mat& src_b = src_b_vec.at(j);
            Color color = flags.at(j);

            vector<RotatedRect> rects_mser;
            rects_mser.reserve(64);
            std::vector<NumberZone> deskewPlate;
            deskewPlate.reserve(64);
            std::vector<NumberZone> mserPlate;
            mserPlate.reserve(64);

            // deskew for rotation and slope image
            for (int i = 0; i < plates.size(); i++)
            {
                NumberZone plate = plates.at(i);
                RotatedRect rrect = plate.getNumberZonePos();
                RotatedRect scaleRect = scaleBackRRect(rrect, (float)scale_ratio);
                plate.setNumberZonePos(scaleRect);
                plate.setNumberZoneColor(color);
            ...
            for (int i = 0; i < deskewPlate.size(); i++)
            {
                NumberZone dplate = deskewPlate.at(i);
                RotatedRect drect = dplate.getNumberZonePos();
                Mat dmat = dplate.getNumberZoneMat();

                for (int k = 0; k < mserPlate.size(); k++)
                {
                    NumberZone splate =  mserPlate.at(k);
                    RotatedRect srect = splate.getNumberZonePos();
                    float iou = 0.f;
                    bool isSimilar = computeIOU(drect, srect, src.cols, src.rows, 0.95f, iou);
                    if (isSimilar)
                    {
                        splate.setNumberZoneMat(dmat);
...
int ZoneLocate::zoneLocate(Mat src, std::vector<Mat>&resultVec, int index)
{
    vector<NumberZone> all_result_zone;

    zoneColorLocate(src, all_result_zone, index);
    zoneSobelLocate(src, all_result_zone, index);
    zoneMserLocate(src, all_result_zone, index);

    for (size_t i = 0; i < all_result_zone.size(); i++) {
        NumberZone zone = all_result_zone[i];
        resultVec.push_back(zone.getNumberZoneMat());
    }

    return 0;
}

int ZoneLocate::zoneLocate(Mat src, std::vector<NumberZone>&resultVec, int index)
{
    std::vector<NumberZone> all_result_zones;

    zoneColorLocate(src, all_result_zones, index);
    zoneSobelLocate(src, all_result_zones, index);
    zoneMserLocate(src, all_result_zones, index);

    for (size_t i = 0; i < all_result_zones.size(); i++) {
        resultVec.push_back(all_result_zones[i]);
    }
